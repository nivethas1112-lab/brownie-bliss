...
const CategoryPage = () => {
  ...
  // Normalize category type (handle hyphens, case)
  const normalizedType = type.toLowerCase().replace(/-/g, '').replace(' ', '-')
  ...
  React.useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const response = await api.products.getByCategory(normalizedType)
        const productList = response?.products || response || []
        const formatted = productList.map((p) => ({
          id: p._id || p.id,
          name: p.name,
          price: p.price,
          img: p.image || p.images?.[0] || '/placeholder-product.jpg',
          rating: p.rating || 5.0
        }))
        setProducts(formatted.length > 0 ? formatted : MOCK_CATEGORY_PRODUCTS[normalizedType] || MOCK_CATEGORY_PRODUCTS.brownies)
      } catch (err) {
        console.warn('Failed to fetch category products, using mock data:', err.message)
        setError('Using cached data')
        setProducts(MOCK_CATEGORY_PRODUCTS[normalizedType] || MOCK_CATEGORY_PRODUCTS.brownies)
      } finally {
        setIsLoading(false)
      }
    }
  ...
  if (isLoading) {
    return (
      <div className="category-page" style={{ minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <Loader size="large" />
        {error && <p style={{ position: 'absolute', bottom: '20px', color: 'var(--text-muted)' }}>{error}</p>}
      </div>
    )
  }
      ...
      <header className="category-header">
        <div className="container">
          <Reveal>
            ...
            <p>Handcrafted {type.toLowerCase().replace('-', ' ')} made with the finest ingredients.</p>
          </Reveal>
        </div>
      </header>
